...
for cliques in cliques_count_list:
    ...
    network_data = pd.DataFrame(columns=['cliques','network_int','connections',
                                     ...
                                     'adaptiveness','degrade', 'copy_player',
                                     'starting_distance_network',
                                     'network_distance',
                                     'clique_1_distance', 'clique_2_distance', 'clique_3_distance',  'clique_4_distance', 'clique_5_distance'])
    ...
    while network_int < max_network_iteration:
        ...
        # Initializations
        copy_player = random.randint(2, 5)
        ...
        connections = 0
        if cliques > 1:
            connections = random.randint(cliques,10) 
        if connections == 0:
            cliques_list= [list(range(pool_players))]
        else:
            cliques_list = base_cliques_generation(pool_players, connections)
        cliques_list = generate_network_cliques(cliques_list.copy(), n=cliques, m=0)
        cliques_connections = generate_connections(connections, pool_players,copy_player)
        cliques_list = cliques_list.copy() + cliques_connections
        print("There will be " + str(cliques) + " and " + str(connections) + " connective groups of which each one contains " + str(copy_player) + " players")
        print("The cliques and connective groups are " + str(cliques_list))
        ...
        starting_clique_4_distance = average_distance_all_arrays(adaptives_network_memory[cliques_list[3]])
        starting_clique_5_distance = average_distance_all_arrays(adaptives_network_memory[cliques_list[4]])
        average_cliques_distance = np.mean([starting_clique_1_distance, starting_clique_2_distance, 
                                            starting_clique_3_distance, starting_clique_3_distance, 
                                            starting_clique_4_distance])
        ...
        clique_4_distance = average_distance_all_arrays(adaptives_network_memory[cliques_list[3]])
        clique_5_distance = average_distance_all_arrays(adaptives_network_memory[cliques_list[4]])
        ...
        if network_int % 100 and network_int > 50:
            file_name_network = "final-0-complexnetwork"+".csv"
...
file_name_network = "final-1-complexnetwork"+".csv"
network_data.to_csv(file_name_network, sep=',', encoding='utf-8', index=False, header=True)
